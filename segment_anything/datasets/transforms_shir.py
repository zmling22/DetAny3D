...
__all__ = ["Compose", "ToTensor", "ToPILImage", "Normalize", "Resize", "ShortEdgeResize", "CenterCrop", "ShortEdgeCenterCrop", "Pad",
           "Lambda", "RandomApply", "RandomChoice", "RandomOrder", "RandomCrop", "RandomHorizontalFlip",
           "RandomVerticalFlip", "RandomResizedCrop", "FiveCrop", "TenCrop",
           "ColorJitter", "RandomRotation", "RandomAffine",
           "RandomPerspective", "RandomCentralCrop"]
...
class ShortEdgeResize(object):
    def __call__(self, img, mask):
        width, height = img.size
        Resize = min(width, height)
        return F.resize(img, Resize, Image.BICUBIC), F.resize(mask, Resize, Image.BICUBIC)
...
class ShortEdgeCenterCrop(object):
    def __call__(self, img, mask):
        width, height = img.size
        crop_size = min(width, height)
        return F.center_crop(img, (crop_size, crop_size)), F.center_crop(mask, (crop_size, crop_size))
...
class RandomCentralCrop(object):
    ...
    @staticmethod
    def get_params(img, min_ratio):
        # Fallback to central crop
        ratio_1 = (1 - min_ratio) * random.random() + min_ratio
        ratio_2 = (1 - min_ratio) * random.random() + min_ratio
        # if img.size[0] < img.size[1]:
        #     ratio_1 = 1
        #     ratio_2 = img.size[0] / img.size[1]
        # else:
        #     ratio_2 = 1
        #     ratio_1 = img.size[1] / img.size[0]
        w = img.size[0] * ratio_1
        h = img.size[1] * ratio_2
        if w < h:
            w_for_test = int(1024 / h * w)
            w -= (w_for_test % 16) // (1024 / h)
        else:
            h_for_test = int(1024 / w * h)
            h -= (h_for_test % 16) // (1024 / w)
    ...
    def __call__(self, img, mask):
        i, j, h, w = self.get_params(img, self.min_ratio)

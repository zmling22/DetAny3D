...
from segment_anything.modeling.adaper import InteractionBlock, SpatialPriorModule, deform_inputs

from timm.models.layers import DropPath, trunc_normal_
from torch.nn.init import normal_
from .unidepth import *
...
def _check_resolution(shape_constraints, resolution_level):
    ...
    clipped_resolution_level = min(max(resolution_level, 0), 10)
    if clipped_resolution_level != resolution_level:
        warnings.warn(
            f"Resolution level {resolution_level} is out of bounds ([0,{10}]). "
            f"Clipping to {clipped_resolution_level}."
        )
    shape_constraints["pixels_bounds"] = [
        pixel_bounds[0]
        + ceil(pixel_range * clipped_resolution_level / 10),
        pixel_bounds[0]
        + ceil(pixel_range * clipped_resolution_level / 10),
    ]
...
def _postprocess(outs, ratio, original_shapes, mode="nearest-exact"):
    outs["depth_maps"] = F.interpolate(outs["depth_maps"], size=original_shapes, mode=mode)
...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.build(cfg)
        if cfg.model.unidepth_head:
            self.depth_head = Unidepth_Decoder(cfg)
        self.cfg = cfg
        self.interpolation_mode = "bilinear"
        self.eps = 1e-6
        self.resolution_level = None
        # self.unidepth_decoder = Decoder(config)
    
    def build(self, config):
        # import ipdb;ipdb.set_trace()
        mod = importlib.import_module("segment_anything.modeling.encoder")
        pixel_encoder_factory = getattr(mod, 'dinov2_vitl14')
        pixel_encoder_config = {'image_shape': [420, 560], 'shape_constraints': {'patch_size': 14, 'pixels_bounds': [1400, 2400], 'pixels_bounds_ori': [1400, 2400], 'ratio_bounds': [0.66, 2.0]}, 'depths': [21, 22, 23, 24], 'embed_dim': 1024, 'embed_dims': [1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024], 'name': 'dinov2_vitl14', 'output_idx': [21, 22, 23, 24], 'patch_size': 14, 'pretrained': None, 'stacking_fn': 'last', 'use_norm': True}
        ...
        self.slices_encoder_range = [(0, 21), (21, 22), (22, 23), (23, 24)]
        self.shape_constraints = {'patch_size': 14, 'pixels_bounds': [1400, 2400], 'pixels_bounds_ori': [1400, 2400], 'ratio_bounds': [0.66, 2.0]}
    ...
    def forward_with_adapter(self, input_dict):
        # import ipdb;ipdb.set_trace()
        rgbs = input_dict['images']
        intrinsics = input_dict['gt_intrinsic']
        B, _, H, W = rgbs.shape
        ...
        tokens = [
            tokens[i:j][-1].contiguous()
            for i, j in self.slices_encoder_range
        ]
        # global_tokens = [cls_tokens[i] for i in [-2, -1]]
        # camera_tokens = [cls_tokens[i] for i in [-3, -2, -1]] + [tokens[-2]]
        # input_dict['image_embeddings'] = x
        input_dict['multi_features'] = features
        input_dict['gt_intrinsic'] = gt_intrinsics
        input_dict['image'] = rgbs
        if self.cfg.model.add_cls_token:
            input_dict['cls_token'] = tokens
        if self.depth_head is not None:
            outs = self.depth_head(input_dict)
            outs = _postprocess(outs, ratio, (H, W), mode=self.interpolation_mode)
            input_dict.update(outs)
            # input_dict['depth_maps'] = depth_map
            # input_dict['incident_maps'] = incident_map
        return input_dict

    def forward(self, input_dict) -> torch.Tensor:
        
        if self.cfg.model.use_adapter:
            return self.forward_with_adapter(input_dict)

        raise NotImplementedError("haven't modify the code without adapter")
        ...
        fea_for_dpt_head = list()

        for i, blk in enumerate(self.blocks):
            x = blk(x)
            if self.depth_head is not None and i in self.take_index:
                fea_for_dpt_head.append(x.permute(0, 3, 1, 2))
        
        ret_dict = {}
        x = self.neck(x.permute(0, 3, 1, 2))
        ret_dict['image_embeddings'] = x
        if self.depth_head is not None:
            depth_map, incident_map = self.depth_head(fea_for_dpt_head)
            ret_dict['depth_maps'] = depth_map
            ret_dict['incident_maps'] = incident_map

...
from .detector_2d_head import Detector2DHead
...
class ImageEncoderViT(nn.Module):
    def __init__(
        ...
        if cfg.model.dpt:
            self.depth_head = DPTHead(1, embed_dim, out_chans)
        elif cfg.model.unidepth_head:
            self.depth_head = Unidepth_Decoder(cfg)
        
        if cfg.model.detector_2d_head:
            self.detector_2d_head = Detector2DHead(cfg)
        
        self.cfg = cfg
    
    def init_adapter(self, cfg, embed_dim, norm_layer):
        # TODO: func init_adapter
    ...
    def forward(self, input_dict) -> torch.Tensor:
        if self.cfg.merge_dino_feature:
            ...
            if self.cfg.add_dino_feature_to_sam_decoder:
                if self.cfg.multi_level_dino:
                    x = dino_features[0] + dino_features[1] + dino_features[2] + dino_features[3]
                else:
                    x = dino_features[-1]
                x = x.permute(0, 3, 1, 2).contiguous()
                x = self.dino_neck(x)
                input_dict['dino_for_sam_decoder'] = x
            input_dict['dino_feature'] = dino_features
            # import ipdb;ipdb.set_trace()
            input_dict['dino_token'] = dino_tokens
        if self.cfg.model.detector_2d_head:
            point_prompts, loss = self.detector_2d_head(dino_features, input_dict['vit_pad_size'], input_dict['gt_masks'])
            if self.cfg.tune_detector_2d_only:
                input_dict['detector_2d_loss'] = loss
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if self.depth_head is not None and i in self.take_index:
                fea_for_depth_head.append(x.permute(0, 3, 1, 2))
        
        ret_dict = {}
        x = self.neck(x.permute(0, 3, 1, 2))
        ret_dict['image_embeddings'] = x
        if self.depth_head is not None:
            depth_map, incident_map = self.depth_head(fea_for_depth_head)
            ret_dict['depth_maps'] = depth_map[:, 0, ...]
...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        #         sns.heatmap(attn[i].cpu().numpy(), cmap='viridis', annot=False, ax=ax)
        ...
        #     # 保存为文件
        #     plt.savefig("attention_heads_4x4.png", dpi=300)

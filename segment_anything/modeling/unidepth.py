...
class Unidepth_Decoder(nn.Module):
    ...
    def forward_with_dino(self, input_dict):
        ...
        # import ipdb;ipdb.set_trace()
        if self.cfg.add_adapter_with_dino:
    ...
    def forward(self, input_dict) -> torch.Tensor:
        if self.cfg.merge_dino_feature:
            return self.forward_with_dino(input_dict)
        B, C, _, _ = input_dict["images"].shape

        assert B == 1

        # image pixel wise shape
        H, W = int(input_dict['images_shape'][0, 0]), int(input_dict['images_shape'][0, 1])

        # feature patch wise shape
        patch_shape_H, patch_shape_W = int(input_dict['vit_pad_size'][0, 0]), int(input_dict['vit_pad_size'][0, 1])

        device = input_dict["images"].device
        dtype = input_dict["images"].dtype
        
        global_tokens = [input_dict['cls_token'][i] for i in [-2, -1]]
        camera_tokens = [input_dict['cls_token'][i] for i in [-3, -2, -1, -2]]
        ...
        features = [input_dict['multi_features'][i][2].permute(0, 2, 3, 1) for i in range(4)]
        level_shapes = sorted(
            list(set([tuple([x.shape[1], x.shape[2]]) for x in features]))
        )[::-1]
        ...
        features_flat = [
            ...
            for x, input_shape in zip(features, input_shapes)
        ]
        features_splits = torch.tensor(
        ...
        # get cls tokens projections
        cls_tokens_splits = torch.tensor(
            [x.shape[-1] for x in camera_tokens],
            device=features.device,
            requires_grad=False,
            dtype=features.dtype,
        )
        camera_tokens = torch.cat(camera_tokens, dim=-1)
        camera_tokens = self.camera_token_adapter(camera_tokens, cls_tokens_splits)
        camera_tokens = torch.cat(
            torch.chunk(camera_tokens, cls_tokens_splits.shape[0], dim=-1), dim=1
        )

        cls_tokens_splits = torch.tensor(
            [x.shape[-1] for x in global_tokens],
            device=features.device,
            requires_grad=False,
            dtype=torch.float32,
        )
        global_tokens = torch.cat(global_tokens, dim=-1)
        global_tokens = self.global_token_adapter(global_tokens, cls_tokens_splits)
        global_tokens = torch.cat(
            torch.chunk(global_tokens, cls_tokens_splits.shape[0], dim=-1), dim=1
        )
        ...
        outputs = {
            "depth_maps": depth,
            "confidence": confidence,
            "depth_features": depth_features.reshape(B, patch_shape_H, patch_shape_W, -1).contiguous(),
            "depth_features_0": depth_features0,
            "metric_features": metric_feature,
            "camera_features": camera_feature,
            "scale": scale,
    ...
    def build(self, cfg):
        ...
        if cfg.merge_dino_feature:
            ...
            self.input_adapter = ListAdapter(cls_token_input_dims, hidden_dim)

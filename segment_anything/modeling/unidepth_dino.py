...
class Unidepth_Decoder(nn.Module):
    ...
    def forward(self, input_dict) -> torch.Tensor:
        B, _, H, W = input_dict["image"].shape
        assert B == 1
        ...
        if input_dict.get('gt_intrinsic') is not None:
            ...
            input_dict["K"] = input_dict['gt_intrinsic'].to(dtype)
        ...
        # if H < W:
        #     short_edge = int(64. / W * H)
        #     features = [input_dict['multi_features'][i][2][..., :short_edge, :].permute(0, 2, 3, 1) for i in range(4)]
        # else:
        #     short_edge = int(64. / H * W)
        #     features = [input_dict['multi_features'][i][2][..., :short_edge].permute(0, 2, 3, 1) for i in range(4)]
        features = input_dict['multi_features']
        level_shapes = sorted(
    ...
    def build(self, cfg):
        ...
        self.slices_encoder = [(20, 21), (21, 22), (22, 23), (23, 24)]
        cls_token_input_dims = [1024, 1024, 1024, 1024]
